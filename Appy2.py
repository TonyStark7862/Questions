...
try:
    ...
    # Try with full path if relative import fails
    import sys
    import os
    sys.path.append(os.path.dirname(os.path.abspath(__file__)))
    from backend import RAGManager

    # Initialize sessions and logs directories
...
def chat_section():
    ...
    # Display chat history (most recent at the bottom)
    ...
    if query and query != st.session_state.get("last_query", ""):
        ...
        with st.spinner("Searching for answers..."):
            ...
            st.experimental_rerun()}:</strong> {source["text"]}</div>'
                                f'<div>'
                                f'<span class="metadata-pill">File: {source["file"]}</span>'
                                f'<span class="metadata-pill">Page: {source["page"]}</span>'
                                f'<span class="metadata-pill">Section: {source["section"]}</span>'
